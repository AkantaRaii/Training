...
from models.dataprocessor import PcaDataProcessor
...
def pca_main(stock,sequence_length ):
    seed = 42
    np.random.seed(seed)
    torch.manual_seed(seed)
    file_path = f'../data/pca_data/p{stock}.csv'
    model_path=f'../best_model/bestmodel_pca/{stock}.pth'
    
    processor = PcaDataProcessor(file_path, sequence_length)
    ...
    print(actual_test[0],predicted_test[0])
    metrics=Metrics(predicted_train,actual_train,predicted_test,actual_test,processor.scaler,processor.close)
